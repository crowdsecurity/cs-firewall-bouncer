...
class TestNFTables(unittest.TestCase):
    ...
    def test_table_rule_set_are_created(self):
        ...
        assert ("ip", "crowdsec4-blacklists", "ipv4_addr") in sets
        rules = {
    ...
    def test_duplicate_decisions_across_decision_stream(self):
        ...
        sleep(1)
        self.assertEqual(get_set_elements("ip", "crowdsec", "crowdsec4-blacklists"), {"0.0.0.0"})

        self.lapi.ds.insert_decisions([d2, d3])
        sleep(1)
        assert self.fb.poll() is None
        self.assertEqual(
            get_set_elements("ip", "crowdsec", "crowdsec4-blacklists"), {"0.0.0.0", "0.0.0.1"}
        )

        self.lapi.ds.delete_decision_by_id(d1["id"])
        self.lapi.ds.delete_decision_by_id(d2["id"])
        sleep(1)
        self.assertEqual(get_set_elements("ip", "crowdsec", "crowdsec4-blacklists"), set())
        assert self.fb.poll() is None
    ...
    def test_decision_insertion_deletion_ipv4(self):
        ...
        set_elements = get_set_elements("ip", "crowdsec", "crowdsec4-blacklists")
    ...
    def test_longest_decision_insertion(self):
        ...
        self.lapi.ds.insert_decisions(decisions)
        sleep(1)
        elems = get_set_elements("ip", "crowdsec", "crowdsec4-blacklists", with_timeout=True)
        assert len(elems) == 1
        elems = list(elems)
